...
class BaseViewController: UIViewController {
    ...
    func getHeaderLabel(title:String,color:UIColor = Colors.primaryLightColor()) -> UILabel{
    ...
    func getGradientHeaderLabel(title:String) -> UILabel{
        let w = title.width(forHeight: 26, font: UIFont(name: "Kanit-Bold", size: 26)!)
        let label = UILabel(frame: CGRect(x: 0, y: 0, width: w, height: 26))
        label.text = title
        label.font = UIFont(name: "Kanit-Bold", size: 26)
